...
function getClient(session?: SessionData): FirecrawlApp {
  ...
  if (process.env.CLOUD_SERVICE === "true") {
    ...
    return createClient(session.luckyApiKey)
  }

  // For self-hosted instances, API key is optional if LUCKY_API_URL is provided
  if (!process.env.LUCKY_API_URL && (!session || !session.luckyApiKey)) {
...
`,
  ...
  execute: async (args: unknown, { session, log }: { session?: SessionData; log: Logger }): Promise<string> => {
...
---|-|-|
| \`"\"\` | Non-fuzzy matches a string of text | \`"Lucky"\`
| \`-\` | Excludes certain keywords or negates other operators | \`-bad\`, \`-site:lucky.dev\`
| \`site:\` | Only returns results from a specified website | \`site:lucky.dev\`
| \`inurl:\` | Only returns results that include a word in the URL | \`inurl:lucky\`
| \`allinurl:\` | Only returns results that include multiple words in the URL | \`allinurl:git lucky\`
| \`intitle:\` | Only returns results that include a word in the title of the page | \`intitle:Lucky\`
| \`allintitle:\` | Only returns results that include multiple words in the title of the page | \`allintitle:lucky playground\`
| \`related:\` | Only returns results that are related to a specific domain | \`related:lucky.dev\`
...
**Optimal Workflow:** Search first using lucky_search without formats, then after fetching the results, use the scrape tool to get the content of the relevantpage(s) that you want to scrape
 ...
 **Usage Example:**
 ...
 \`\`\`
 **Returns:** Operation ID for status checking; use lucky_check_crawl_status to check progress.
