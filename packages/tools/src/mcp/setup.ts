	...
	try {
		...
		if (path.isAbsolute(command)) {
			return (
				...
				(fs.statSync(command).mode & fs.constants.X_OK) !== 0
			)
		}
...
function loadExternalMCPConfig(
	configPath?: string,
	...
	try {
		...
		for (const [name, serverConfig] of Object.entries(mcpConfig.mcpServers)) {
			if (!serverConfig.command || !Array.isArray(serverConfig.args)) {
				errors.push(
					`  ❌ ${name}: missing 'command' or 'args' (must be an array)`,
				)
				continue
			}
			...
			const processedConfig = {
				...
				args: serverConfig.args.map((arg) =>
					substituteEnvVars(arg, missingVars),
				),
				env: serverConfig.env
					? Object.fromEntries(
							Object.entries(serverConfig.env).map(([key, value]) => [
								key,
								substituteEnvVars(value, missingVars),
							]),
						)
					: undefined,
			}
			...
			if (missingVars.length > 0) {
				errors.push(
					`  ⚠️  ${name}: missing environment variables: ${missingVars.join(", ")}`,
				)
			}
		...
		if (Object.keys(validatedServers).length > 0) {
			console.log(
				`✅ Loaded MCP servers: ${Object.keys(validatedServers).join(", ")}\n`,
			)
		} else if (Object.keys(mcpConfig.mcpServers).length > 0) {
			console.warn(
				"⚠️  No valid MCP servers found (all configurations failed validation)\n",
			)
		}
...
export async function setupMCPForNode(
	...
	const safeToolNames: MCPToolName[] = Array.isArray(toolNames)
		? toolNames
		: []
	...
	for (let i = 0; i < clientResults.length; i++) {
		...
		if (result.status === "fulfilled" && result.value !== null) {
			...
			console.error(
				`\n❌ MCP client connection failed for '${toolName}':`,
				result.reason,
			)
		} else {
			failedTools.push(toolName)
	...
	if (failedTools.length > 0) {
		...
		if (successfulClients.length > 0) {
			console.warn(
				`✅ Working tools: ${successfulClients.map((c) => c.name).join(", ")}`,
			)
		} else {
	...
	const toolSetResults = await Promise.allSettled(
		successfulClients.map(async ({ name, client }) => {
			try {
				...
				console.error(
					`\n❌ Failed to fetch tools for MCP client '${name}':`,
					error,
				)
...
export function clearWorkflowMCPClientCache(workflowId: string) {
	...
	if (keysToDelete.length > 0) {
		console.log(
			`🧹 Cleared ${keysToDelete.length} cached MCP client(s) for workflow ${workflowId}`,
		)
	}
}
...
export function getMCPStatus(configPath?: string): {
	...
	const createTools = getCreateTools(configPath)
	const resolvedPath =
		configPath ?? process.env.MCP_SECRET_PATH ?? undefined
...
export function logMCPStatus(configPath?: string): void {
	...
	if (status.configured) {
		console.log(
			`🔧 Available servers: ${status.availableServers.join(", ")}`,
		)
