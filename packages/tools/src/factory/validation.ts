...
// Simple console logger - tools package is standalone
const lgg = {
  warn: console.warn,
  error: console.error,
}
...
export function validateAndCorrectWithSchema<T>(
  ...
  for (const issue of validationResult.error.issues) {
    ...
    if (issue.code === "too_big" && issue.type === "number" && typeof issue.maximum === "number") {
      ...
      lgg.warn("Auto-correcting tool parameter", {
        toolName,
        parameter: paramPath,
        originalValue,
      ...
      lgg.error("Uncorrectable validation issue", {
        toolName,
        parameter: paramPath,
        value: originalValue,
        issue: issue.message,
  ...
  if (corrected) {
    ...
    if (!revalidation.success) {
      lgg.error("Auto-correction failed - parameters still invalid", {
        toolName,
