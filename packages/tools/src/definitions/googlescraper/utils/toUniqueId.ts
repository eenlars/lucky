...
function areBusinessesDuplicates(business1: GoogleMapsBusiness, business2: GoogleMapsBusiness): boolean {
  // primary checks (high confidence)
  if (business1.placeId && business2.placeId) {
    return business1.placeId === business2.placeId
  }

  if (business1.address && business2.address && business1.storeName && business2.storeName) {
    return (
      normalizeAddress(business1.address) === normalizeAddress(business2.address) &&
      normalizeStoreName(business1.storeName) === normalizeStoreName(business2.storeName)
    )
  }

  if (business1.phone && business2.phone && business1.storeName && business2.storeName) {
    return (
  ...
  if (business1.googleUrl && business2.googleUrl) {
    return extractGoogleUrlId(business1.googleUrl) === extractGoogleUrlId(business2.googleUrl)
  }

  return false
}

export function deduplicateBusinesses(businesses: GoogleMapsBusiness[]): GoogleMapsBusiness[] {
  const unique: GoogleMapsBusiness[] = []

  for (const business of businesses) {
    const isDuplicate = unique.some(existing => areBusinessesDuplicates(business, existing))
