...
async function runOnce(model: string, condition: Condition) {
  ...
  const result = await runMultiToolOurAlgorithm(model as any, scenario.prompt, adaptiveTools, sys, {})
  ...
  for (const e of toolExecs) {
    ...
    console.log(`  • ${e.toolName} args=${JSON.stringify(e.inputData)} retType=${retType}`)
  }
  ...
  console.log(`- Total cost (USD): ${Number(result.totalCostUsd || 0).toFixed(4)}`)
