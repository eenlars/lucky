...
export function parseWorkflowResultToMetrics(result: InvokeWorkflowResult): Metrics {
  ...
  return {
    ...
    time: queueRunResult.totalTime
      ? `${(queueRunResult.totalTime / 1000).toFixed(1)}s`
      : fitness?.totalTimeSeconds
        ? `${fitness.totalTimeSeconds.toFixed(1)}s`
        : null,
    cost: fitness?.totalCostUsd
      ? `$${fitness.totalCostUsd.toFixed(4)}`
      : usdCost
        ? `$${usdCost.toFixed(4)}`
        : queueRunResult.totalCost
          ? `$${queueRunResult.totalCost.toFixed(4)}`
          : null,
