...
export function useWorkflowProgress(invocationId?: string) {
  const { events } = useWorkflowStream({
    invocationId,
    events: ['workflow:started', 'workflow:progress', 'workflow:completed', 'node:execution:completed']
  })
  ...
  useEffect(() => {
    ...
    const nodeCompletions = events.filter(e => e.event === 'node:execution:completed')
    ...
    if (workflowStarted) {
      ...
      const latestCompletion = nodeCompletions[nodeCompletions.length - 1] as any
      const currentNodeId = latestProgress?.currentNodeId || latestCompletion?.nodeId || null
...
export function useNodeEvents(nodeId: string, invocationId?: string) {
  ...
  const nodeEvents = events.filter((event: any) => 
    event.nodeId === nodeId || event.toNodeId === nodeId
  )

  const isExecuting = nodeEvents.some((event: any) => 
    event.event === 'node:execution:started' && 
    !nodeEvents.some((e: any) => 
  ...
  return {
    ...
    isExecuting,
    lastExecution,
    toolCalls: nodeEvents.filter((event: any) => event.event.startsWith('tool:')),
