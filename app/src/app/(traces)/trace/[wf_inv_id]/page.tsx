    ...
    return nodeInvocations.map((inv, idx) => {
      ...
      if (idx === nodeInvocations.length - 1 && inv.output) {
        ...
        output = {
          ...
          seq: 0,
          role: "result",
          payload: typeof inv.output === "string" ? { kind: "result", workDone: inv.output } : inv.output,
  ...
  const calculatePerformanceMetrics = (workflow: Tables<"WorkflowInvocation">, timeline: FullTraceEntry[]) => {
    ...
    const totalCost =
      workflow.usd_cost && workflow.usd_cost > 0
        ? workflow.usd_cost
        : timeline.reduce((sum, entry) => sum + (entry.invocation.usd_cost || 0), 0)
    ...
    timeline.forEach(entry => {
      ...
      if (duration > maxDuration) {
        ...
        bottleneckNode = entry.nodeDefinition.node_id || `Node ${entry.invocation.node_id}`
      }
    })
