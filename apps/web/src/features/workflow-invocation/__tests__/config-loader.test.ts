...
import { loadWorkflowConfigFromInput } from "../lib/config-loader"
...
describe("loadWorkflowConfigFromInput", () => {
  ...
  it("should load from file when filename is provided", async () => {
    ...
    const input: InvocationInput = {
      filename: "/path/to/workflow.json",
      evalInput: {
        type: "prompt-only",
        goal: "test",
        workflowId: "test",
  ...
  it("should load from database when workflowVersionId is provided", async () => {
    const { loadWorkflowConfig } = await import("../lib/database-workflow-loader")
  ...
  it("should prioritize dslConfig over filename", async () => {
    const mockConfig = {
      __schema_version: 1,
      entryNodeId: "node1",
      nodes: [],
    ...
    // Testing priority - having both dslConfig and filename (implementation uses dslConfig first)
    const input = {
      dslConfig: mockConfig,
      filename: "/path/to/workflow.json",
      evalInput: {
    ...
    // @ts-expect-error - Intentionally testing invalid state with both dslConfig and filename
    const result = await loadWorkflowConfigFromInput(input)
  ...
  it("should prioritize filename over workflowVersionId", async () => {
    const { readFile } = await import("node:fs/promises")
  ...
  it("should handle file read errors gracefully", async () => {
    ...
    const input: InvocationInput = {
      filename: "/nonexistent/workflow.json",
      evalInput: {
        type: "prompt-only",
        goal: "test",
        workflowId: "test",
  ...
  it("should handle database load failure gracefully", async () => {
    ...
    const input: InvocationInput = {
      workflowVersionId: "wf_ver_nonexistent",
      evalInput: {
        type: "prompt-only",
        goal: "test",
        workflowId: "test",
  ...
  it("should load complex workflow config from file", async () => {
    ...
    const input: InvocationInput = {
      filename: "/path/to/complex-workflow.json",
      evalInput: {
        type: "prompt-only",
        goal: "test",
        workflowId: "test",
