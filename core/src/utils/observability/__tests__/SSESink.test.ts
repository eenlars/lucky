...
import { WorkflowEventBuilder, AsyncTestUtils } from '@core/__tests__/test-data-builders'
...
class MockStreamController {
  ...
  enqueue(data: Uint8Array | string) {
    ...
    const stringData = data instanceof Uint8Array 
...
describe.skip('SSESink - Server-Sent Events Sink', () => {
  ...
  describe('Connection Management', () => {
    ...
    it('should handle multiple connections', () => {
      ...
      const invocation1Clients = sink.getConnections()
        .filter(conn => conn.invocationId === 'invocation-1')
    ...
    it('should mark connections as disconnected when controller closes', () => {
      ...
      const connections = sink.getConnections()
      expect(connections).toHaveLength(1)
      expect(connections[0].connected).toBe(true) // Will be marked disconnected on next event
    })
  })
  ...
  describe('Event Broadcasting', () => {
    it('should broadcast events to all connected clients', () => {
      ...
      sink.addConnection(mockController1 as any, 'client-1', 'broadcast-test', [])
      sink.addConnection(mockController2 as any, 'client-2', 'broadcast-test', [])
    ...
    it('should filter events by invocationId', () => {
      ...
      sink.addConnection(mockController1 as any, 'client-A', 'invocation-A', [])
      sink.addConnection(mockController2 as any, 'client-B', 'invocation-B', [])
    ...
    it('should handle events without invocationId (broadcast to all)', () => {
      ...
      sink.addConnection(mockController1 as any, 'client-1', 'invocation-1', [])
      sink.addConnection(mockController2 as any, 'client-2', 'invocation-2', [])
  ...
  describe('Event Formatting', () => {
    it('should format SSE events correctly', () => {
      ...
      sink.addConnection(mockController1 as any, 'format-test', 'test-invocation', [])
    ...
    it('should handle events with special characters', () => {
      ...
      sink.addConnection(mockController1 as any, 'special-chars', 'special-test', [])
  ...
  describe('Error Handling', () => {
    ...
    it('should handle malformed events', () => {
      // Arrange
      sink.addConnection(mockController1 as any, 'malformed-test', 'test-inv', [])
    ...
    it('should clean up disconnected clients during broadcast', () => {
      // Arrange
      sink.addConnection(mockController1 as any, 'client-1', 'test-inv', [])
      sink.addConnection(mockController2 as any, 'client-2', 'test-inv', [])
  ...
  describe('Performance & Memory Management', () => {
    it('should handle many simultaneous connections', () => {
      ...
      for (let i = 0; i < connectionCount; i++) {
        ...
        sink.addConnection(
          controller as any,
          `client-${i}`,
          `invocation-${i % 10}`, // 10 different invocations
          []
        )
      }
      ...
      const receivedCount = controllers.filter(c => c.enqueued.length > 0).length
    ...
    it('should handle high-frequency event emission', () => {
      ...
      sink.addConnection(mockController1 as any, 'high-freq-client', 'test-inv', [])
