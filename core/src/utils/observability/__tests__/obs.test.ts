...
import { obs, setSink, MemorySink, StdoutSink, TeeSink, withSpan } from '@core/utils/observability/obs'
import type { Sink } from '@core/utils/observability/obs'
import { WorkflowEventBuilder, ContextBuilder, AsyncTestUtils } from '@core/__tests__/test-data-builders'
...
describe('obs - Core Observability Module', () => {
  ...
  describe('Span Management', () => {
    it('should create and manage spans correctly', async () => {
      ...
      // Act
      await obs.scope(context, async () => {
        await withSpan(undefined, 'test-operation', { operationType: 'test' }, async () => {
