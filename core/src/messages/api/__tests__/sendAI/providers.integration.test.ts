...
const isRealKey = (key: string) =>
  Boolean(key && !key.trim().toLowerCase().startsWith("test-"))

const availableProviders = (Object.keys(providerKey) as Provider[]).filter(
  (p) => isRealKey(providerKey[p])
)

// Minimal prompts
const prompts = [
  "Reply with one short sentence about testing.",
...
// Skip by default unless explicitly enabled to avoid network flakiness
describe.skipIf(
  availableProviders.length === 0 ||
    process.env.ENABLE_PROVIDER_INTEGRATION !== "true"
)("sendAI provider smoke (.integration.test)", () => {
