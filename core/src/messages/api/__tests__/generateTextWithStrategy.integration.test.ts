...
import { createPrepareStepStrategy } from "@core/tools/any/selectToolStrategy"
...
describe("generateText with createPrepareStepStrategy", () => {
  it("should execute tools in sequence using step strategy: tool1 -> tool2 -> tool3", async () => {
    ...
    const tool1Result = resultV2?.outputs.find((r) => r.name === "nod_333")
    const tool2Result = resultV2?.outputs.find((r) => r.name === "mod_888")
    const tool3Result = resultV2?.outputs.find((r) => r.name === "rod_999")
