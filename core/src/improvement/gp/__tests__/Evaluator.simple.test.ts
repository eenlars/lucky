...
vi.mock("@core/improvement/evaluators/AggregatedEvaluator", () => ({
  AggregatedEvaluator: class {
    evaluate = vi.fn().mockResolvedValue({
      success: true,
      data: {
        fitness: { score: 0.8, totalCostUsd: 0.05, totalTimeSeconds: 1.5, accuracy: 0.85, novelty: 0.85 },
...
vi.mock("@core/improvement/gp/resources/tracker", () => ({
  failureTracker: { trackEvaluationAttempt: vi.fn(), trackEvaluationFailure: vi.fn() },
}))
...
describe("GPEvaluatorAdapter", () => {
  ...
  it("evaluates genome successfully", async () => {
    const { GPEvaluatorAdapter } = await import("@core/improvement/evaluators/GPEvaluatorAdapter")
    ...
    const result = await evaluator.evaluate(genome, {
      runId: "test-run", generationId: "test-gen", generationNumber: 1,
    })
