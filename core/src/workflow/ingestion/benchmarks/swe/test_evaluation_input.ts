...
async function downloadSWEBenchDataset(): Promise<void> {
  if (!HF_TOKEN) {
    console.log("⚠️  HUGGING_FACE_API_KEY not found, skipping dataset download")
    return
  }

  console.log("📥 Downloading SWE-bench dataset...")
      ...
      try {
        ...
        if (datasetFiles.length > 0) {
          ...
          try {
            ...
            console.log(`✅ Dataset file already exists: ${tempPath}`)
            return
          } catch {
          ...
          console.log(`✅ Downloaded dataset to: ${tempPath}`)
          return
        }
      } catch (error) {
        console.log(
...
async function validateAISolution(
  ...
  console.log("\n🔍 Validating AI solution against ground truth...")
...
Be thorough in your analysis and provide a working solution.`
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      console.log("💰 Cost:", `$${response.usdCost?.toFixed(4) || "0.0000"}`)
      ...
      console.log("\n📊 Confidence:", `${structuredResponse.confidence}/10`)
      ...
      console.log("❌ AI call failed:", response.error)
      throw new Error(response.error || "AI call failed")
    }

    return response
  } catch (error) {
    console.error("❌ Error during AI solution generation:", error)
    throw error
  }
}
...
async function testSWEBenchEvaluationInput(
  ...
  console.log("🔬 Testing SWE-bench Evaluation Input")
  console.log("=".repeat(50))
  ...
  try {
    ...
    console.log("\n🔄 Converting evaluation input to workflow cases...")
    const workflowCases = await IngestionLayer.convert(evaluationInput)

    console.log(`✅ Generated ${workflowCases.length} workflow case(s)`)
    ...
    console.log("\n🎉 Evaluation input test completed successfully!")
    console.log("=".repeat(50))
...
async function runEvaluationInputTests() {
  console.log("🚀 Starting SWE-bench Evaluation Input Tests")
  console.log("=".repeat(60))
