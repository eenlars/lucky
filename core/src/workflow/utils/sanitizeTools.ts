...
export function sanitizeConfigTools(config: WorkflowConfig): WorkflowConfig {
  ...
  const sanitizedNodes: WorkflowNodeConfig[] = config.nodes.map((node) => {
    const nextMcp = (Array.isArray(node.mcpTools) ? node.mcpTools : []).filter(
      (t): t is MCPToolName => mcpAllowed.has(t as MCPToolName)
    )
    const nextCode = (
      Array.isArray(node.codeTools) ? node.codeTools : []
    ).filter((t): t is CodeToolName => codeAllowed.has(t as CodeToolName))
